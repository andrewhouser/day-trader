...
- **Two-tier navigation** — nav bar restructured from a flat list of 17 links into 5 top-level group tabs (Dashboard, Trading, Analysis, History, System) with inline sub-tabs. Active group expands to show its sub-tabs below the main bar. Reduces visual clutter while keeping all pages one click away.
- **Project structure reorganization** — moved 12 agent modules into `agents/` subdirectory and 9 core library modules into `core/` subdirectory. Root directory now contains only entry points (`api.py`, `server.py`, `scheduler.py`), configuration (`config.py`), and infrastructure files. All imports updated across the codebase including lazy/inline imports. Dockerfile simplified from 5 COPY lines to 3 using directory-level copies. Entrypoint commands updated to use `agents.*` import paths.
- **Position rotation** — the trading agent can now sell underperforming positions to fund better opportunities. When a high-scoring buy opportunity is identified but cash is insufficient, the agent scores all held positions and sells the weakest one to free capital, provided the new opportunity scores at least 2 composite points higher. When cash drops below 15%, the prompt explicitly nudges the agent to consider rotation rather than passing on the trade. No position has loyalty — every holding must continuously earn its place.
...
- **PortfolioChart build failure** — `react/jsx-sort-props` ESLint rule required alphabetically sorted props on the new `<Line>` elements; `key` was listed before `connectNulls`.
